...
const QuestionPage = () => {
  ...
  const handleMCQSelection = (answer) => {
    ...
    const currentQuestionNumber = parseInt(questionNumber);
    console.log(
      'Current Question Number in handleMCQSelection:',
  ...
  const handleSubmit = (event) => {
    ...
    const currentQuestionNumber = parseInt(questionNumber);
    console.log(
      'Current Question Number in handleSubmit:',
      currentQuestionNumber
    );
    ...
    if (currentQuestionNumber === 8) {
      ...
      const recommendationMap = {
        ...
        Acne: ['Acne Treatment Gel', 'Benzoyl Peroxide Face Wash'],
        default: ['General Skin Care Cream', 'Hydrating Face Mask'],
      };

      const recommendations =
        recommendationMap[userAnswer] || recommendationMap['default'];
  ...
  const currentQuestion = questions.find(
    (q) => q.id === parseInt(questionNumber)
  );
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="button-container">
            {parseInt(questionNumber) > 1 && (
              <button
                type="button"
                className="quiz-button1 back-button"
                onClick={handleBack}
              >
                ← BACK
              </button>
            ...
            {currentQuestion.id === 3 && (
              <button
                type="button"
                className="quiz-button1 skip-button"
                onClick={handleSkip}
              >
                SKIP →
              </button>
            ...
            <button
              className="quiz-button1 next-button"
              type="submit"
              disabled={!userAnswer && !email && !phone}
            >
              NEXT →
            </button>
          </div>
